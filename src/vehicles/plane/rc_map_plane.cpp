...
void RCMapPlane::process() {
    ...
    for (int ch = 0; ch < 16; ch++) {
        ...
        if (source_id >= 0 && source_id < 200) {
            int16_t out = get_tuned_val(state.tuned_slots[source_id], false, state.inverted[ch]);
            state.channels[ch] = out;
            state.mapped_channels[ch] = out; // CRITICAL BRIDGE
        }
    }
    for (int i = 0; i < 6; i++) {
        int target = state.split_mixers[i].target_ch;
        if (target >= 0 && target < 16) {
            int32_t pos = get_tuned_val(state.tuned_slots[state.split_mixers[i].pos_id],
                                        state.split_mixers[i].pos_center, state.split_mixers[i].pos_reverse);
            int32_t neg = get_tuned_val(state.tuned_slots[state.split_mixers[i].neg_id],
                                        state.split_mixers[i].neg_center, state.split_mixers[i].neg_reverse);
            state.mapped_channels[target] = static_cast<int16_t>(std::max(-32768, std::min(32767, (int)(pos + neg))));
        }
    }
}
