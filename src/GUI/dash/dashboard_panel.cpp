...
#include "vehicles/car/input_tune_car_panel.h"
#include "vehicles/car/main_car_dash.h"
#include "vehicles/car/rc_map_car_panel.h"
#include "vehicles/car/rc_tune_car_panel.h"
#include "vehicles/drone/input_tune_drone_panel.h"
#include "vehicles/drone/main_drone_dash.h"
#include "vehicles/drone/rc_map_drone_panel.h"
#include "vehicles/drone/rc_tune_drone_panel.h"
#include "vehicles/heli/input_tune_heli_panel.h"
#include "vehicles/heli/main_heli_dash.h"
#include "vehicles/heli/rc_map_heli_panel.h"
#include "vehicles/heli/rc_tune_heli_panel.h"
...
#include "vehicles/plane/rc_tune_plane_panel.h"
...
DashboardPanel::DashboardPanel(QWidget *parent) : QWidget(parent)
{
    ...
    // Shield: Made invisible per your preference (Alpha = 0)
    ...
    QList<QWidget *> panels = {input_gui, settings_gui, rc_type_gui, profile_gui,
                               rc_stack,  input_stack,  rc_map_stack};
    for (auto p : panels) {
    ...
    connect(button_grid->inner_settings_btn, &QPushButton::clicked, this,
            &DashboardPanel::toggle_settings_gui);
    connect(button_grid->rc_type_btn, &QPushButton::clicked, this,
            &DashboardPanel::toggle_rc_type_gui);
    connect(button_grid->profile_btn, &QPushButton::clicked, this,
    ...
    connect(rc_type_gui->getSelector(), &RCTypeSelector::typeChanged, this,
            &DashboardPanel::onVehicleTypeChanged);
    connect(&ThemeManager::instance(), &ThemeManager::themeChanged, this,
            &DashboardPanel::refresh_theme);
...
void DashboardPanel::update_data()
{
    this->update();
}
...
void DashboardPanel::refresh_theme()
{
    ...
    sidebar->setStyleSheet(
        QString("QFrame { background-color: rgba(5, 5, 5, 220); border-left: 2px solid %1; }")
            .arg(accent));
}
...
void DashboardPanel::update_panel_positions()
{
    ...
    int ty = (h - 600) / 2;
    QList<QWidget *> ps = {input_gui, settings_gui, rc_type_gui, profile_gui,
                           rc_stack,  input_stack,  rc_map_stack};
...
void DashboardPanel::hide_all_guis()
{
    ...
    input_stack->hide();
    rc_map_stack->hide();
    input_open = settings_open = rc_type_open = profile_open = rc_tune_open = input_tune_open =
        rc_map_open = false;
}
