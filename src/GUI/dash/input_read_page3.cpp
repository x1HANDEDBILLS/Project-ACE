...
class NeonBar : public QWidget
{
   public:
    ...
    void refresh()
    {
        // Port 3 starts at offset 100 in the 200-slot array
   ...
   protected:
    void paintEvent(QPaintEvent*) override
    {
        ...
        // Map 16-bit range (-32768 to 32767) to visual bar
...
void setup_input_read_page3_content(QWidget* p)
{
    auto* main_layout = new QVBoxLayout(p);
    main_layout->setContentsMargins(15, 10, 15, 10);

    QLabel* status = new QLabel("PORT 3 STATUS");
    ...
    // PORT 3 ANALOG (100-107)
    v_box->addWidget(createHeader("PORT 3 ANALOG (100-107)", theme));
    for (int i = 0; i < 8; i++) {
    ...
    // PORT 3 MOTION (130-138)
    v_box->addWidget(createHeader("PORT 3 MOTION (130-138)", theme));
    QStringList m_labs = {"TRK ROLL", "TRK PITCH", "TRK YAW", "ACC X", "ACC Y",
                          "ACC Z",    "GYR X",     "GYR Y",   "GYR Z"};
    for (int i = 0; i < 9; i++) {
        auto* b = new NeonBar(container);
        b->label = m_labs[i];
        b->slot_index = 130 + i;
    ...
    // PORT 3 BUTTONS (108-129)
    v_box->addWidget(createHeader("PORT 3 BUTTONS (108-129)", theme));
    ...
    QObject::connect(t, &QTimer::timeout, [=]() {
        ...
        for (int i = 0; i < labels.size(); i++) {
            ...
            labels[i]->setStyleSheet(
                on ? QString("background:%1; color:black; font-weight:bold;").arg(theme.name())
                   : "background:#080808; color:#444; border:1px solid #1a1a1a; font-size:7px;");
        }
    });
    t->start(16);
