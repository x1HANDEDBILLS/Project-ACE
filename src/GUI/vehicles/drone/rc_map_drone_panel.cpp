// LOCATION: /home/user/ACE/src/GUI/vehicles/drone/rc_map_drone_panel.cpp
...
RCMapDronePanel::RCMapDronePanel(QWidget *parent) : QFrame(parent)
{
    ...
    status_label = new QLabel("// RC MAP: DRONE");
    header_title = new QLabel("DRONE SOURCE MAPPING");
    ...
    page_titles = {"SOURCES", "MODES", "TRIMS", "CURVES", "FAILSAFE"};
    for (int i = 0; i < 5; ++i) {
    ...
    connect(prev_btn, &QPushButton::clicked, this, &RCMapDronePanel::prev_page);
    connect(next_btn, &QPushButton::clicked, this, &RCMapDronePanel::next_page);
    update_page_ui();
    refresh_theme();
}
void RCMapDronePanel::update_page_ui()
{
    int idx = pages->currentIndex();
    header_title->setText(page_titles[idx]);
    page_label->setText(QString("%1 / 5").arg(idx + 1));
}
void RCMapDronePanel::next_page()
{
    pages->setCurrentIndex((pages->currentIndex() + 1) % 5);
    update_page_ui();
}
void RCMapDronePanel::prev_page()
{
...
void RCMapDronePanel::refresh_theme()
{
...
RCMapDronePanel::~RCMapDronePanel() {}
