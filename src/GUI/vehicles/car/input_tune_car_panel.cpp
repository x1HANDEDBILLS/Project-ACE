// LOCATION: /home/user/ACE/src/GUI/vehicles/car/input_tune_car_panel.cpp
...
#include "theme.h"
#include "vehicles/car/input_tune_car_page1.h"
#include "vehicles/car/input_tune_car_page2.h"
#include "vehicles/car/input_tune_car_page3.h"
#include "vehicles/car/input_tune_car_page4.h"
#include "vehicles/car/input_tune_car_page5.h"

InputTuneCarPanel::InputTuneCarPanel(QWidget *parent) : QFrame(parent)
{
    ...
    status_label = new QLabel("// VEHICLE: ROVER/CAR");
    header_title = new QLabel("CAR TUNING");
    ...
    page_titles = {"STEERING", "THROTTLE", "BRAKING", "SKID-STEER", "FAILSAFE"};
    for (int i = 0; i < 5; ++i) {
        QWidget *p = new QWidget();
        if (i == 0)
            setup_input_tune_car_page1_content(p);
        else if (i == 1)
            setup_input_tune_car_page2_content(p);
        else if (i == 2)
            setup_input_tune_car_page3_content(p);
        else if (i == 3)
            setup_input_tune_car_page4_content(p);
        else if (i == 4)
            setup_input_tune_car_page5_content(p);
        pages->addWidget(p);
    }
    ...
    connect(prev_btn, &QPushButton::clicked, this, &InputTuneCarPanel::prev_page);
    connect(next_btn, &QPushButton::clicked, this, &InputTuneCarPanel::next_page);
    update_page_ui();
    refresh_theme();
}

void InputTuneCarPanel::update_page_ui()
{
    int idx = pages->currentIndex();
    header_title->setText(page_titles[idx]);
    page_label->setText(QString("%1 / 5").arg(idx + 1));
}
void InputTuneCarPanel::next_page()
{
    pages->setCurrentIndex((pages->currentIndex() + 1) % 5);
    update_page_ui();
}
void InputTuneCarPanel::prev_page()
{
...
void InputTuneCarPanel::refresh_theme()
{
...
InputTuneCarPanel::~InputTuneCarPanel() {}
