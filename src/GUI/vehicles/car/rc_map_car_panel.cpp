// LOCATION: /home/user/ACE/src/GUI/vehicles/car/rc_map_car_panel.cpp
...
RCMapCarPanel::RCMapCarPanel(QWidget *parent) : QFrame(parent)
{
    ...
    status_label = new QLabel("// RC MAP: GROUND");
    header_title = new QLabel("CAR SOURCE MAPPING");
    ...
    page_titles = {"SOURCES", "STEERING", "TRIMS", "CURVES", "FAILSAFE"};
    for (int i = 0; i < 5; ++i) {
    ...
    connect(prev_btn, &QPushButton::clicked, this, &RCMapCarPanel::prev_page);
    connect(next_btn, &QPushButton::clicked, this, &RCMapCarPanel::next_page);
    update_page_ui();
    refresh_theme();
}
void RCMapCarPanel::update_page_ui()
{
    int idx = pages->currentIndex();
    header_title->setText(page_titles[idx]);
    page_label->setText(QString("%1 / 5").arg(idx + 1));
}
void RCMapCarPanel::next_page()
{
    pages->setCurrentIndex((pages->currentIndex() + 1) % 5);
    update_page_ui();
}
void RCMapCarPanel::prev_page()
{
...
void RCMapCarPanel::refresh_theme()
{
