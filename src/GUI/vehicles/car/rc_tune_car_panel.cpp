// LOCATION: /home/user/ACE/src/GUI/vehicles/car/rc_tune_car_panel.cpp
...
#include "theme.h"
#include "vehicles/car/rc_tune_car_page1.h"
#include "vehicles/car/rc_tune_car_page2.h"
#include "vehicles/car/rc_tune_car_page3.h"
#include "vehicles/car/rc_tune_car_page4.h"
#include "vehicles/car/rc_tune_car_page5.h"

RCTuneCarPanel::RCTuneCarPanel(QWidget *parent) : QFrame(parent)
{
    ...
    status_label = new QLabel("// RC LINK: CAR");
    header_title = new QLabel("CAR RC CALIBRATION");
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        if (i == 0)
            setup_rc_tune_car_page1_content(p);
        else if (i == 1)
            setup_rc_tune_car_page2_content(p);
        else if (i == 2)
            setup_rc_tune_car_page3_content(p);
        else if (i == 3)
            setup_rc_tune_car_page4_content(p);
        else if (i == 4)
            setup_rc_tune_car_page5_content(p);
        pages->addWidget(p);
    }
    ...
    connect(prev_btn, &QPushButton::clicked, this, &RCTuneCarPanel::prev_page);
    connect(next_btn, &QPushButton::clicked, this, &RCTuneCarPanel::next_page);
    update_page_ui();
    refresh_theme();
}
void RCTuneCarPanel::update_page_ui()
{
    int idx = pages->currentIndex();
    header_title->setText(page_titles[idx]);
    page_label->setText(QString("%1 / 5").arg(idx + 1));
}
void RCTuneCarPanel::next_page()
{
    pages->setCurrentIndex((pages->currentIndex() + 1) % 5);
    update_page_ui();
}
void RCTuneCarPanel::prev_page()
{
...
void RCTuneCarPanel::refresh_theme()
{
