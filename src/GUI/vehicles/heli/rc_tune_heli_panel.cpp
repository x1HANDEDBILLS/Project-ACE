// LOCATION: /home/user/ACE/src/GUI/vehicles/heli/rc_tune_heli_panel.cpp
...
#include "vehicles/heli/rc_tune_heli_page4.h"
#include "vehicles/heli/rc_tune_heli_page5.h"

RCTuneHeliPanel::RCTuneHeliPanel(QWidget *parent) : QFrame(parent)
{
    ...
    status_label = new QLabel("// RC LINK: HELI");
    header_title = new QLabel("HELI RC CALIBRATION");
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        if (i == 0)
            setup_rc_tune_heli_page1_content(p);
        else if (i == 1)
            setup_rc_tune_heli_page2_content(p);
        else if (i == 2)
            setup_rc_tune_heli_page3_content(p);
        else if (i == 3)
            setup_rc_tune_heli_page4_content(p);
        else if (i == 4)
            setup_rc_tune_heli_page5_content(p);
        pages->addWidget(p);
    }
    ...
    connect(prev_btn, &QPushButton::clicked, this, &RCTuneHeliPanel::prev_page);
    connect(next_btn, &QPushButton::clicked, this, &RCTuneHeliPanel::next_page);
    update_page_ui();
    refresh_theme();
}
void RCTuneHeliPanel::update_page_ui()
{
    int idx = pages->currentIndex();
    header_title->setText(page_titles[idx]);
    page_label->setText(QString("%1 / 5").arg(idx + 1));
}
void RCTuneHeliPanel::next_page()
{
    pages->setCurrentIndex((pages->currentIndex() + 1) % 5);
    update_page_ui();
}
void RCTuneHeliPanel::prev_page()
{
...
void RCTuneHeliPanel::refresh_theme()
{
...
RCTuneHeliPanel::~RCTuneHeliPanel() {}
