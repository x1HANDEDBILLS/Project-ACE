// LOCATION: /home/user/ACE/src/GUI/vehicles/heli/input_tune_heli_panel.cpp
...
#include "theme.h"
#include "vehicles/heli/input_tune_heli_page1.h"
#include "vehicles/heli/input_tune_heli_page2.h"
#include "vehicles/heli/input_tune_heli_page3.h"
#include "vehicles/heli/input_tune_heli_page4.h"
#include "vehicles/heli/input_tune_heli_page5.h"

InputTuneHeliPanel::InputTuneHeliPanel(QWidget *parent) : QFrame(parent)
{
    ...
    status_label = new QLabel("// VEHICLE: HELICOPTER");
    header_title = new QLabel("HELI TUNING");
    ...
    page_titles = {"COLLECTIVE", "CYCLIC", "TAIL ROTOR", "GOVERNOR", "FAILSAFE"};
    for (int i = 0; i < 5; ++i) {
        QWidget *p = new QWidget();
        if (i == 0)
            setup_input_tune_heli_page1_content(p);
        else if (i == 1)
            setup_input_tune_heli_page2_content(p);
        else if (i == 2)
            setup_input_tune_heli_page3_content(p);
        else if (i == 3)
            setup_input_tune_heli_page4_content(p);
        else if (i == 4)
            setup_input_tune_heli_page5_content(p);
        pages->addWidget(p);
    }
    ...
    connect(prev_btn, &QPushButton::clicked, this, &InputTuneHeliPanel::prev_page);
    connect(next_btn, &QPushButton::clicked, this, &InputTuneHeliPanel::next_page);
    update_page_ui();
    refresh_theme();
}
void InputTuneHeliPanel::update_page_ui()
{
    int idx = pages->currentIndex();
    header_title->setText(page_titles[idx]);
    page_label->setText(QString("%1 / 5").arg(idx + 1));
}
void InputTuneHeliPanel::next_page()
{
    pages->setCurrentIndex((pages->currentIndex() + 1) % 5);
    update_page_ui();
}
void InputTuneHeliPanel::prev_page()
{
...
void InputTuneHeliPanel::refresh_theme()
{
...
InputTuneHeliPanel::~InputTuneHeliPanel() {}
