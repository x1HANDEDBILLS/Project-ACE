// LOCATION: /home/user/ACE/src/GUI/vehicles/plane/rc_tune_plane_panel.cpp
...
#include "theme.h"
#include "vehicles/plane/rc_tune_plane_page1.h"
#include "vehicles/plane/rc_tune_plane_page2.h"
#include "vehicles/plane/rc_tune_plane_page3.h"
#include "vehicles/plane/rc_tune_plane_page4.h"
#include "vehicles/plane/rc_tune_plane_page5.h"

RCTunePlanePanel::RCTunePlanePanel(QWidget *parent) : QFrame(parent)
{
    ...
    status_label = new QLabel("// RC LINK: PLANE");
    header_title = new QLabel("PLANE RC CALIBRATION");
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        if (i == 0)
            setup_rc_tune_plane_page1_content(p);
        else if (i == 1)
            setup_rc_tune_plane_page2_content(p);
        else if (i == 2)
            setup_rc_tune_plane_page3_content(p);
        else if (i == 3)
            setup_rc_tune_plane_page4_content(p);
        else if (i == 4)
            setup_rc_tune_plane_page5_content(p);
        pages->addWidget(p);
    }
    ...
    connect(prev_btn, &QPushButton::clicked, this, &RCTunePlanePanel::prev_page);
    connect(next_btn, &QPushButton::clicked, this, &RCTunePlanePanel::next_page);
    update_page_ui();
    refresh_theme();
}
void RCTunePlanePanel::update_page_ui()
{
    int idx = pages->currentIndex();
    header_title->setText(page_titles[idx]);
    page_label->setText(QString("%1 / 5").arg(idx + 1));
}
void RCTunePlanePanel::next_page()
{
    pages->setCurrentIndex((pages->currentIndex() + 1) % 5);
    update_page_ui();
}
void RCTunePlanePanel::prev_page()
{
...
void RCTunePlanePanel::refresh_theme()
{
...
RCTunePlanePanel::~RCTunePlanePanel() {}
