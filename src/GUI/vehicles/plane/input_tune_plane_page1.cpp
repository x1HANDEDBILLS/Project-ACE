...
class ResponseCurve : public QWidget
{
   public:
    ...
    ResponseCurve(QWidget* p) : QWidget(p)
    {
...
class AxisRow : public QWidget
{
   ...
   private slots:
    void openKeypad()
    {
        auto* kp = new TacticalKeypad(AppState::instance().deadzones[idx]);
        kp->show();
        QObject::connect(kp, &TacticalKeypad::submitted, [this, kp](double v) {
...
class TuningPageController : public QObject
{
   ...
   public:
    ...
    TuningPageController(QObject* p) : QObject(p)
    {
        connect(&ProfileManager::instance(), &ProfileManager::profileLoaded, this,
                &TuningPageController::refreshUI);
    }
   ...
   public slots:
    void refreshUI()
    {
        ...
        if (expo_title)
            expo_title->setText(QString("GLOBAL EXPO CURVE: %1%").arg(state.current_expo));
...
void setup_input_tune_plane_page1_content(QWidget* p)
{
    ...
    QString headStyle =
        "color: white; font-weight: bold; font-size: 16px; border-bottom: 2px solid #333; "
        "padding-bottom: 5px;";
    ...
    save_btn->setStyleSheet(
        "background: #00aa00; color: white; font-weight: bold; border-radius: 5px;");
    ...
    QObject::connect(timer, &QTimer::timeout, [ctrl, graph]() {
        ...
        for (int i = 0; i < ctrl->axis_rows.size(); i++) {
            ctrl->axis_rows[i]->updateValues(dev.axes[i]);
        }
