...
class Page1Manager : public QObject
{
   public:
    ...
    void buildUI()
    {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            name->setStyleSheet(
                "color: #555; font-size: 11px; font-weight: bold; text-align: center;");
    ...
    void refresh()
    {
        ...
        for (int i = 0; i < buttons.size(); i++) {
            ...
            if (is_overridden) {
                ...
                b->setEnabled(false);
                b->setStyleSheet(
                    "background:#080808; color:#444; border:1px solid #222; font-size:10px; "
                    "font-weight:bold;");
            } else {
   ...
   protected:
    ...
    {
        if (obj == parentWidget && ev->type() == QEvent::Show) {
            refresh();
        }
