...
RCMapPlanePage3::RCMapPlanePage3(QWidget* parent) : QWidget(parent)
{
    ...
    for (int i = 0; i < 16; i++) {
        ...
        name->setStyleSheet(
            "color: #555; font-size: 11px; font-weight: bold; font-family: 'Consolas';");
        name->setAlignment(Qt::AlignCenter);

        rev_buttons[i] = new QPushButton();
        rev_buttons[i]->setFixedHeight(50);
    ...
    // Connect to global signals for persistence and theme switching
    connect(&ProfileManager::instance(), &ProfileManager::profileLoaded, this,
            &RCMapPlanePage3::refresh_ui);
        ...
        if (inv) {
            rev_buttons[i]->setStyleSheet(
                QString("background: %1; color: black; font-weight: bold; border: none; font-size: "
                        "16px; font-family: 'Consolas';")
                    .arg(accent));
        } else {
            rev_buttons[i]->setStyleSheet(
                "background: #222; color: #777; border: 1px solid #333; font-size: 16px; "
