// LOCATION: /home/user/ACE/src/GUI/widgets/keypad.cpp
...
TacticalKeypad::TacticalKeypad(double initial_val, QWidget *parent) : QWidget(parent)
{
    ...
    QStringList keys_text = {"7", "8", "9", "4",   "5",    "6",      "1",    "2",
                             "3", ".", "0", "CLR", "BACK", "CANCEL", "ENTER"};
    ...
    for (const QString &txt : keys_text) {
        ...
        connect(btn, &QPushButton::clicked, this, &TacticalKeypad::handle_key);
        keys.append(btn);
...
void TacticalKeypad::handle_key()
{
    Button *btn = qobject_cast<Button *>(sender());
    if (!btn) return;
    QString t = btn->text();
    if (t == "ENTER")
        emit submitted(display->text().toDouble());
    else if (t == "CANCEL")
        emit cancelled();
    else if (t == "CLR")
        display->clear();
...
void TacticalKeypad::refresh_theme()
{
    ...
    for (Button *b : keys) {
        if (b->text() == "ENTER")
            b->setStyleSheet(
                QString("QPushButton{background:%1; color:black; font-weight:bold;}").arg(h));
        if (b->text() == "CANCEL")
            b->setStyleSheet("QPushButton{background:#441111; color:white;}");
    }
}
