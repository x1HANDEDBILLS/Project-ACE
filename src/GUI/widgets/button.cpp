// Location: src/GUI/widgets/button.cpp
#include "theme.h"
...
Button::Button(const QString &text, QWidget *parent, bool small) : QPushButton(text, parent), m_small(small) {
    if (m_small) {
        setFixedSize(100, 35);
        m_defaultFontSize = 9;
...
void Button::regenerateCache() {
    ...
    for (int y = 0; y < h; ++y) {
        ...
        for (int x = 0; x < w; ++x) {
            ...
            int r =
                qBound(0, (int)(cfg.bg_light.red() + (cfg.bg_dark.red() - cfg.bg_light.red()) * ratio + noise), 255);
            int g = qBound(
                0, (int)(cfg.bg_light.green() + (cfg.bg_dark.green() - cfg.bg_light.green()) * ratio + noise), 255);
            int b =
                qBound(0, (int)(cfg.bg_light.blue() + (cfg.bg_dark.blue() - cfg.bg_light.blue()) * ratio + noise), 255);
            *line++ = (uchar)r;
            *line++ = (uchar)g;
            *line++ = (uchar)b;
        }
...
void Button::paintEvent(QPaintEvent *event) {
    ...
    // The "Cool" Standard Path (Clipped Corners)
    QPainterPath path;
    path.moveTo(e, 0);
    path.lineTo(w - e, 0);
    path.lineTo(w, e);
