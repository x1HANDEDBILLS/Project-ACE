// LOCATION: /home/user/ACE/src/GUI/widgets/keyboard.cpp
...
TacticalKeyboard::TacticalKeyboard(const QString &initial_text, QWidget *parent) : QWidget(parent)
{
    ...
    for (int r = 0; r < 5; ++r) {
        ...
        for (const QString &key : rows[r]) {
            ...
            connect(btn, &QPushButton::clicked, this, &TacticalKeyboard::handle_key);
            keys.append(btn);
...
void TacticalKeyboard::handle_key()
{
    Button *btn = qobject_cast<Button *>(sender());
    QString key = btn->text();
    if (key == "ENTER")
        emit submitted(display->text());
    else if (key == "CANCEL")
        emit cancelled();
    else if (key == "BACK")
...
void TacticalKeyboard::refresh_theme()
{
    ...
    for (Button *b : keys) {
        if (b->text() == "ENTER")
            b->setStyleSheet(QString("QPushButton{background:%1; color:black;}").arg(h));
        if (b->text() == "CANCEL")
