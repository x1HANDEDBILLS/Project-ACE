...
void NomadHardware::start()
{
    ...
    // PIN THREAD TO CORE 3 (Pi 4 has Cores 0,1,2,3)
    // This ensures the GUI (usually Core 0/1) doesn't cause lag
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    CPU_SET(3, &cpuset);
...
void NomadHardware::run_loop()
{
    ...
    while (running.load()) {
        // TX - Exactly like original nomad_daemon
        ...
        if (ser != -1) write(ser, pkt, 26);

        // RX - Match original poll/read behavior
        uint8_t rb[256];
        ssize_t b = read(ser, rb, 256);
        if (b > 0) {
            ...
            while (head + 2 < t_buf.size()) {
                if (t_buf[head] == 0xEE || t_buf[head] == 0xEA || t_buf[head] == 0xC8) {
                    ...
                    if (len < 2 || head + len + 2 > t_buf.size()) {
                        head++;
                        continue;
                    }
                    if (t_buf[head + len + 1] == internal_crsf_crc(&t_buf[head + 2], len - 1)) {
                        ...
                        if (type == 0x14) {
                            ...
                            state.tele.lat =
                                (int32_t)((d[0] << 24) | (d[1] << 16) | (d[2] << 8) | d[3]) / 1e7;
                            state.tele.lon =
                                (int32_t)((d[4] << 24) | (d[5] << 16) | (d[6] << 8) | d[7]) / 1e7;
