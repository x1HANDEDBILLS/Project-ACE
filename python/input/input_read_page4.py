...
class InputReadPage4(QWidget):
    def __init__(self, parent=None):
        ...
        # 1. Header Status Display - PORT 4
        self.status_label = QLabel("PORT 4: WAITING...")
    ...
    def create_widgets(self, num_axes, num_btns, has_motion):
        ...
        raw_group_header.setStyleSheet("color: #00E5FF; font-weight: bold; font-size: 12px; margin-top: 5px; margin-bottom: 5px;")
        ...
        if has_motion:
            ...
            label_style = "color: #00FFFF; font-family: 'Consolas', monospace; font-size: 12px; border: none;"
    ...
    def update_state(self, proc):
        """Processes live data specifically from Slot 4."""
        if not proc: return
        try:
            slots = proc.get_slots() 
            p4 = slots.get("4") # TARGET SLOT 4
            
            is_connected = p4 and p4.get("conn", False)

            if is_connected:
                device_name = p4.get('name', 'HID Device')
                axes = p4.get('axes', [])
                btns = p4.get('btns', [])
                accel = p4.get('accel', [])
                gyro = p4.get('gyro', [])
                ...
                if self.current_device != device_name or len(self.axis_bars) != len(axes):
                    self.status_label.setText(f"PORT 4: {device_name.upper()}")
                    self.status_label.setStyleSheet("color: #00E5FF; font-family: 'Consolas'; font-size: 20px; font-weight: bold;")
                ...
                for i, state in enumerate(btns):
                    if i < len(self.btn_indicators):
                        ...
                        if is_pressed != self.btn_states[i]:
                            ...
                            if is_pressed:
                                self.btn_indicators[i].setStyleSheet("background: #00E5FF; color: #000; border-radius: 4px; font-weight: bold;")
                            else:
                                self.btn_indicators[i].setStyleSheet("background: #222; color: #444; border-radius: 4px; font-weight: bold;")
            else:
                if self.current_device is not None:
                    self.status_label.setText("PORT 4: DISCONNECTED")
