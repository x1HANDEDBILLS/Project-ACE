...
class InputReadPage3(QWidget):
    def __init__(self, parent=None):
        ...
        # 1. Header Status Display - PORT 3
        self.status_label = QLabel("PORT 3: WAITING...")
        ...
        self.btn_states = [] # Cache to minimize CSS overhead
    ...
    def create_widgets(self, num_axes, num_btns, has_motion):
        ...
        raw_group_header.setStyleSheet("color: #BB00FF; font-weight: bold; font-size: 12px; margin-top: 5px; margin-bottom: 5px;")
        ...
        if num_axes > 0:
            ...
            for i in range(num_axes):
                ...
                bar.setStyleSheet("""
                    ...
                    QProgressBar::chunk { background-color: #BB00FF; }
                """)
        ...
        if has_motion:
            ...
            label_style = "color: #FF00FF; font-family: 'Consolas', monospace; font-size: 12px; border: none;"
    ...
    def update_state(self, proc):
        """Processes live data specifically from Slot 3."""
        if not proc: return
        try:
            slots = proc.get_slots() 
            p3 = slots.get("3") # TARGET SLOT 3
            
            is_connected = p3 and p3.get("conn", False)

            if is_connected:
                device_name = p3.get('name', 'HID Device')
                axes = p3.get('axes', [])
                btns = p3.get('btns', [])
                accel = p3.get('accel', [])
                gyro = p3.get('gyro', [])
                ...
                if self.current_device != device_name or len(self.axis_bars) != len(axes):
                    self.status_label.setText(f"PORT 3: {device_name.upper()}")
                    self.status_label.setStyleSheet("color: #BB00FF; font-family: 'Consolas'; font-size: 20px; font-weight: bold;")
                ...
                for i, state in enumerate(btns):
                    if i < len(self.btn_indicators):
                        ...
                        if is_pressed != self.btn_states[i]:
                            ...
                            if is_pressed:
                                self.btn_indicators[i].setStyleSheet("background: #BB00FF; color: #000; border-radius: 4px; font-weight: bold;")
                            else:
                                self.btn_indicators[i].setStyleSheet("background: #222; color: #444; border-radius: 4px; font-weight: bold;")
            else:
                if self.current_device is not None:
                    self.status_label.setText("PORT 3: DISCONNECTED")
