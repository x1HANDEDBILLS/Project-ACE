...
class SourceSelector : public QDialog
{
   public:
    ...
    SourceSelector(int current, QWidget* parent = nullptr) : QDialog(parent)
    {
        ...
        setStyleSheet(
            "background: #050508; border: 2px solid #444; border-radius: 10px; color: white;");
        ...
        QObject::connect(t, &QTimer::timeout, [this]() {
            ...
            for (int i = 0; i < 50; i++) {
                ...
                bool moving = (std::abs(val) > 100);
                gBtns[i]->setStyleSheet(
                    moving ? QString("border: 2px solid %1; background: #1a1a25; color:white;")
                                 .arg(theme_color)
